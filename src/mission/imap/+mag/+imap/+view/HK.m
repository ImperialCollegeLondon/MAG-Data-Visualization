...
% HK Show HK of instrument.
    ...
    methods
        ...
        function visualize(this)
            ...
            if sid5.isPlottable()
                ...
                if ~isempty(sid5FOBTable)

                    nSID5 = nSID5 + 1;
                    charts = [charts, {sid5}, this.getSID5Charts(sid5FOBTable, primarySensor, "left")];
                end

                if ~isempty(sid5FIBTable)

                    nSID5 = nSID5 + 1;
                    charts = [charts, {sid5}, this.getSID5Charts(sid5FIBTable, secondarySensor, "right")];
                end
    ...
    methods (Static, Access = private)
        ...
        function charts = getSID5Charts(data, sensor, yAxisLocation)

            charts = { ...
                mag.graphics.style.Stackedplot(Title = sensor, YLabels = ["x [nT]", "y [nT]", "z [nT]", "|B| [nT]"], YAxisLocation = yAxisLocation, Layout = [3, 1], Charts = mag.graphics.chart.Stackedplot(XVariable = sensor + "T", YVariables = sensor + ["X", "Y", "Z", "B"])), ...
                data, mag.graphics.style.Default(Title = compose("%s Modes", sensor), YLabel = "mode [-]", YLimits = "manual", YAxisLocation = yAxisLocation, Charts = mag.imap.chart.Event(EventOfInterest = "Rate", IgnoreMissing = false)), ...
                data, mag.graphics.style.Default(Title = compose("%s Ranges", sensor), YLabel = "range [-]", YLimits = "manual", YAxisLocation = yAxisLocation, Charts = mag.imap.chart.Event(EventOfInterest = "Range", IgnoreMissing = false, YOffset = 0.25)), ...
                data, mag.graphics.style.Default(Title = compose("%s Compression", sensor), YLabel = "compressed [-]", YLimits = "manual", YAxisLocation = yAxisLocation, Charts = mag.imap.chart.Event(EventOfInterest = "Compression", IgnoreMissing = false, YOffset = 0.1))};
        end
    end
end
